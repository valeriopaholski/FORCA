...
sk.on('letter',({room,l})=>{let r=R(room);let cur=r.players[r.turn];if(cur?.id!==sk.id)return;l=l.toLowerCase();r.players.forEach(p=>{if(p.id!==sk.id){if(!r.guesses[p.id].includes(l)){r.guesses[p.id].push(l);if(!r.words[p.id].includes(l))r.errors[p.id]++}}});r.turn=(r.turn+1)%r.players.length;io.to(room).emit('state',r)});
sk.on('guess',({room,w})=>{let r=R(room);let cur=r.players[r.turn];if(cur?.id!==sk.id)return;w=w.toLowerCase();r.players.forEach(p=>{if(p.id!==sk.id){if(w!==r.words[p.id])r.errors[p.id]=6}});r.turn=(r.turn+1)%r.players.length;io.to(room).emit('state',r)});
});
srv.listen(3000);
